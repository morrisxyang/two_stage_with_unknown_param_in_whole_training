...
def IPOfunc(cur_opt=None,A =None,b =None,h=None,cTrue=None,GTrue=None,purchase_fee=0,compensation_fee=0,alpha0=0.9995,beta=0.1,pc = True,
...
#                print("violateFactor: ", violateFactor, "violateEdgeIndex: ", violateEdgeIndex)
                ...
                if method==3: #only rows
                    ctx.save_for_backward(cur_opt,x_v,
...
#            print('Solve LP cost:',time_end1-time_start,'s')
        ...
        def backward(ctx,del_x):
            ...
            nonlocal run_time
            start = time.time()
            cur_opt,x,y,t,tau,kappa,c,A,b,trueA,trueb,trueG,trueh = ctx.saved_tensors
#            print(x_sol,violateFactor,violateEdgeIndex)
...
#            print('Compute dxG cost:',time_end3-time_end2,'s')
            ...
            dLxs1 = np.zeros(var_num)
            for i in range(var_num):
                if cur_opt[i] > x[i]:
                    dLxs1[i] = - compensation_fee
                else:
                    dLxs1[i] = compensation_fee
...
#            dRxs2 = dRxs2 * del_x.numpy()
            ...
            dlG = dLxs1.dot(dxG)
            dlG = torch.from_numpy(dlG)
            dG = torch.matmul(dlG, del_x)
#            print("dG1: ", dxG1 * del_x)
#            print("dG2: ", dxG2 * del_x)

            dlc = dLxs1.dot(dxc_numpy)
            dlc = torch.from_numpy(dlc)
            dc = torch.matmul(dlc, del_x)
#            print("dc1: ", dxc1 * del_x)
#            print("dc2: ", dxc2 * del_x)
#            print(dG1, dG2, dc1, dc2)
