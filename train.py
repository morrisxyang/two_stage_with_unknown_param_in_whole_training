...
# Updated data path to use custom_datasets_700
current_folder_path = os.path.abspath(os.getcwd())
data_base_path = os.path.join(current_folder_path, 'data/custom_datasets_700/knapsack')

# Create output directories for results (just two mkdir calls)
run_timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
run_base_rel = f"results/{run_timestamp}-cap={capacity}-purchase={purchase_fee}-compensation={compensation_fee}"
prices_rel = f"{run_base_rel}/proposed_prices"
weights_rel = f"{run_base_rel}/proposed_weights"

# Ensure only the two leaf directories are created (parents auto-created)
create_folder.mkdir(current_folder_path, prices_rel)
create_folder.mkdir(current_folder_path, weights_rel)

# Store paths for later file outputs
store_price_path = os.path.join(current_folder_path, prices_rel)
store_weight_path = os.path.join(current_folder_path, weights_rel)

# check correct
def load_knapsack_data(data_path, cap, num_train_samples=700, seed=None, random_select=True):
    if seed is not None:
        np.random.seed(seed)
    ...
    train_encodings = np.load(os.path.join(data_path, 'train_encodings.npy'), mmap_mode='r')  # (N, 10, 4096)
    train_instances = np.load(os.path.join(data_path, 'train_instances.npy'))  # (N, 10, 2) [weight, price]
    train_sols = np.load(os.path.join(data_path, f'train_sols_cap{cap}.npy'))  # (N, 10)

    # Load test data
    test_encodings = np.load(os.path.join(data_path, 'test_encodings.npy'), mmap_mode='r')
    test_instances = np.load(os.path.join(data_path, 'test_instances.npy'))
    test_sols = np.load(os.path.join(data_path, f'test_sols_cap{cap}.npy'))

    # Select training samples (random or sequential)
    total_train = train_encodings.shape[0]
    if num_train_samples > total_train:
        print(f"Warning: requested {num_train_samples} samples but only {total_train} available")
        num_train_samples = total_train

    if random_select:
        indices = np.random.choice(total_train, size=num_train_samples, replace=False)
        indices.sort()  # better memory access pattern
        enc_sel = np.array(train_encodings[indices])
        inst_sel = train_instances[indices].copy()
        sols_sel = train_sols[indices].copy()
        selection_desc = f"randomly selected from {total_train}"
    else:
        # sequential selection: take the first N
        enc_sel = np.array(train_encodings[:num_train_samples])
        inst_sel = train_instances[:num_train_samples].copy()
        sols_sel = train_sols[:num_train_samples].copy()
        selection_desc = "sequentially selected (first N)"

    # Copy selected data into memory
    train_data = {
        'encodings': enc_sel,  # (N, 10, 4096)
        'instances': inst_sel,  # (N, 10, 2)
        'optimal_sols': sols_sel  # (N, 10)
    }

    test_data = {
        'encodings': np.array(test_encodings),
        'instances': test_instances.copy(),
        'optimal_sols': test_sols.copy()
    }

    print(f"Loaded {num_train_samples} training samples ({selection_desc})")
    print(f"Loaded {test_data['encodings'].shape[0]} test samples")
    print(f"Using capacity: {cap}")
...
# check correct, batch get optimal value array
# TODO improve style
...
# check-correct, get optimal sols
# TODO: improve style
...
# check-correct, get second stage optimal value
# TODO: improve style
# error in sol, but not return, should use directly x2 as sol
...
def silu(input):
    ...
    for i in range(item_num):
        if output[i][0] < 0:
            output[i][0] = 0
        output[i][0] = output[i][0] + ReLUValue
        if output[i][1] < 0:
            output[i][1] = 0
        output[i][1] = output[i][1] + ReLUValue
    return output
...
def make_fc(num_layers, num_features, num_targets=target_num,
    ...
    net_layers: list[nn.Module] = [nn.Linear(num_features, intermediate_size), activation_fn()]
    for hidden in range(num_layers - 2):
        net_layers.append(nn.Linear(intermediate_size, intermediate_size))
        net_layers.append(activation_fn())
    net_layers.append(nn.Linear(intermediate_size, num_targets))
    # Remove Softplus activation to get raw price/weight predictions
    # net_layers.append(activation_function)
    net_layers.append(nn.ReLU())
    # # Constrain outputs to price[10,45], weight[15,35]
    # net_layers.append(nn.Sigmoid())
    # net_layers.append(PriceWeightScaler(price_min=10.0, price_max=45.0,
    #                                     weight_min=15.0, weight_max=35.0))
...
class Intopt:
    def __init__(self, opt_sol, h, A, b, purchase_fee, compensation_fee, n_features, num_layers=5, smoothing=False,
                 thr=0.1, max_iter=None, method=1, mu0=None,
            ...
            if e < 1:
                ...
                for feature, value in train_dl:
                    ...
                    while torch.min(op) <= 0 or torch.isnan(op).any() or torch.isinf(op).any():
                        ...
                        # ensure optimizer tracks new model parameters after reinit
                        self.optimizer = self.optimizer.__class__(self.model.parameters(), **self.hyperparams)
                        op = self.model(feature).squeeze()
                    ...
                    x_s1 = IPOfunc(cur_opt=cur_opt_sol, A=A_torch, b=b_torch, h=h_torch, cTrue=-c_torch, GTrue=G_torch,
                                   purchase_fee=self.purchase_fee, compensation_fee=self.compensation_fee,
                                   max_iter=self.max_iter, thr=self.thr, damping=self.damping,
    ...
    def val_loss(self, cap, feature, value):
        ...
        #        itemVal = self.c
        itemVal = valueTemp[:, 0]
        real_obj = actual_obj(itemVal, cap,
                              valueTemp[:, 1], n_instance=int(test_instance))
        ...
        obj_list = []
        corr_obj_list = []
        total_len = np.size(valueTemp, 0)
        predVal = torch.zeros((total_len, 2))
...
def train(run_times: int, random_select=True, lr=1e-5):
    ...
    for testi in range(run_times):
        print(f"\n=== Test run {testi} ===")
        # Load data with a different random seed for each test run
        train_data, test_data = load_knapsack_data(
            data_base_path,
            cap=capacity,
            num_train_samples=train_case_num,
            random_select=random_select,
            # seed=testi  # Different random sample for each test run
        )

        # Prepare training data
        # train_data['encodings']: (700, 10, 4096)
        # train_data['instances']: (700, 10, 2) where [:,:,0] is weight, [:,:,1] is price
        # train_data['optimal_sols']: (700, 10)

        # Reshape encodings: (700, 10, 4096) -> (7000, 4096)
        x_train = train_data['encodings'].reshape(-1, feature_num)
        print(f'x_train.shape: {x_train.shape}')

        # Extract prices and weights: (700, 10, 2) -> (7000, 2)
        # Note: instances format is [weight, price], need to swap to [price, weight]
        train_instances_flat = train_data['instances'].reshape(-1, 2)
        y_train = np.zeros((train_instances_flat.shape[0], 2))
        y_train[:, 0] = train_instances_flat[:, 1]  # price
        y_train[:, 1] = train_instances_flat[:, 0]  # weight
        print(f'y_train.shape: {y_train.shape}')

        train_feature = torch.from_numpy(x_train).float()
        print(f'train_feature.shape: {train_feature.shape}')
        train_value = torch.from_numpy(y_train).float()
        print(f'train_value.shape: {train_value.shape}')

        # Prepare test data
        x_test = test_data['encodings'].reshape(-1, feature_num)
        print(f'x_test.shape: {x_test.shape}')
        test_instances_flat = test_data['instances'].reshape(-1, 2)
        y_test = np.zeros((test_instances_flat.shape[0], 2))
        y_test[:, 0] = test_instances_flat[:, 1]  # price
        y_test[:, 1] = test_instances_flat[:, 0]  # weight
        print(f'y_test.shape: {y_test.shape}')

        # For validation - extract individual price and weight arrays
        test_true_prices = y_test[:, 0]  # prices
        test_true_weights = y_test[:, 1]  # weights
        ...
        print(f'value_test.shape: {value_test.shape}')

        # Optimal solutions: keep shape (700, 10) for batch processing
        # Each row is one instance's solution (10 items)
        train_opt_sols = train_data['optimal_sols']
        print(f'train_opt_sols.shape: {train_opt_sols.shape}')
        test_opt_sols = test_data['optimal_sols']
        print(f'test_opt_sols.shape: {test_opt_sols.shape}')
        ...
        for j in range(5):
            ...
            if avgTrainCorrReg < stopCriterior:
                print(f"avgTrainCorrReg:{avgTrainCorrReg} < stopCriterior:{stopCriterior}")
                break
        ...
        for i in range(predTestVal1.size):
            ...
            predValuePrice[i][0] = test_true_prices[i]
            predValuePrice[i][1] = predTestVal1[i]

        price_file_path = os.path.join(store_price_path, 'proposed_prices(' + str(testi) + ').txt')
        np.savetxt(price_file_path, predValuePrice, fmt="%.2f")
        print(f"✓ Saved: {price_file_path}")

        predValueWeight = np.zeros((predTestVal2.size, 2))
        for i in range(predTestVal2.size):
            #        predValue[i][0] = int(i/item_num)
            predValueWeight[i][0] = test_true_weights[i]
            predValueWeight[i][1] = predTestVal2[i]

        weight_file_path = os.path.join(store_weight_path, 'proposed_weights(' + str(testi) + ').txt')
        np.savetxt(weight_file_path, predValueWeight, fmt="%.2f")
        print(f"✓ Saved: {weight_file_path}")

        from utils import generate_and_save_solutions_for_run
        # Generate and save solutions (predicted and true) for this run
        generate_and_save_solutions_for_run(predTestVal, test_data['instances'], capacity,
                                            current_folder_path, run_base_rel)

        HSD_rslt = 'test: ' + str(np.mean(val_rslt))
        print(HSD_rslt)
        print('Elapsed time: ' + str(end - start))
        recordBest[0][testi] = np.sum(val_rslt)
        print(f"=== Test run {testi} completed ===\n")

    print(recordBest)


def clear_files():
    # Clean up old model file before training
    model_pkl_path = os.path.join(current_folder_path, 'model.pkl')
    if os.path.exists(model_pkl_path):
        os.remove(model_pkl_path)
        print(f"Removed old model file: {model_pkl_path}")



# # 强制cuDNN使用确定性算法，确保结果可复现
# torch.backends.cudnn.deterministic = True
# # 禁用cuDNN自动调优，避免不同运行中算法选择不同
